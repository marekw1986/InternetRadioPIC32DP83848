...
void uart_init (void) {
    ...
    INTEnable(INT_SOURCE_UART_RX(UART1B), INT_DISABLED);    //DISABLED for now - debig why incomming uart data causes reset. U4 instead U1B?
    OpenUART1B(UART_EN | UART_NO_PAR_8BIT | UART_1STOPBIT | UART_DIS_BCLK_CTS_RTS, UART_RX_DISABLE | UART_TX_ENABLE, SYSCLK/16/UART1BAUD-1);
}
