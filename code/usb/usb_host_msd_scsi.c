...
#ifdef RCACHE
static unsigned char cache[512*RCACHE];
static unsigned long scache[RCACHE]; // sectors in cache
    ...
    if (deviceAddress == address)
    {
        switch( (INT) event )
        {
            ...
            case EVENT_MSD_RESET:
                #ifdef DEBUG_MODE
                    printf( "SCSI: MSD Reset performed.\n" );
                #endif
                return TRUE;
                break;
            ...
            case EVENT_MSD_MAX_LUN:
                #ifdef DEBUG_MODE
                    printf( "SCSI: Max LUN set.\n" );
                #endif
                mediaInformation.maxLUN                     = *((BYTE *)data);
				 printf( "SCSI: LUN %d\n",mediaInformation.maxLUN );
...
MEDIA_INFORMATION * USBHostMSDSCSIMediaInitialize( void )
{
    ...
    while (attempts != 0)
    {
        ...
        if (!errorCode)
        {
            ...
            #ifdef DEBUG_MODE
				 printf( "SCSI: Sector size: %d%d%d%d\n",inquiryData[7],inquiryData[6], inquiryData[5], inquiryData[4] );
            #endif
...
BYTE USBHostMSDSCSISectorRead( DWORD sectorAddress, BYTE *dataBuffer )
{
    ...
    #ifdef DEBUG_MODE
		 printf( "SCSI: Reading sector 0x%x device 0x%x\n",sectorAddress,deviceAddress );
    #endif
	...
	if(!firstread){
		for(i=0;i<RCACHE;i++){
			if(scache[i]==sectorAddress) {
				memcpy(dataBuffer,&cache[512*i],512);
			//	printf("cache hit %x\n",sectorAddress);
				return TRUE;
			}
		}
 ...
 //   commandBlock[8] = RCACHE;
    ...
    #ifdef RCACHE
	commandBlock[8] = RCACHE;
	for(i=0;i<RCACHE;i++) scache[i]=sectorAddress+i;
    errorCode = USBHostMSDRead( deviceAddress, 0, commandBlock, 10, cache, mediaInformation.sectorSize*RCACHE );
	#else
	commandBlock[8] = 0x1;
	errorCode = USBHostMSDRead( deviceAddress, 0, commandBlock, 10, dataBuffer, mediaInformation.sectorSize );
	#endif

	//  errorCode = USBHostMSDRead( deviceAddress, 0, commandBlock, 10, dataBuffer, mediaInformation.sectorSize*RCACHE );
...
//	printf("s=%x\n",sectorAddress);
    ...
    } else  printf( "SCSI: Read sector init error 0x%x\n",errorCode );


    if (!errorCode)
    {
		#ifdef RCACHE
		memcpy(dataBuffer,cache,512);
		#endif
        return TRUE;
    }
    else
    {
		printf("SCSI: Read sector error %d byteCount %d \n",errorCode,byteCount);
...
//BYTE USBHostMSDSCSISectorWrite( DWORD sectorAddress, BYTE *dataBuffer, BYTE allowWriteToZero )
BYTE USBHostMSDSCSISectorWrite( DWORD sectorAddress, BYTE *dataBuffer)
{
    DWORD   byteCount;
    BYTE    commandBlock[10];
    BYTE    errorCode;
    ...
    #ifdef DEBUG_MODE
        printf( "SCSI: Writing sector 0x%x device 0x%x\n",sectorAddress,deviceAddress );
	...
	#ifdef RCACHE
           for(i=0;i<RCACHE;i++)
                if(scache[i]==sectorAddress)  memcpy(&cache[512*i],dataBuffer,512); // found the same sector in cache, update it
    #endif
    ...
    } else  printf( "SCSI: Write sector error 0x%x\n", errorCode );
