...
uint16_t lcd_utf8str_part(const char* str, const uint16_t len) {
    ...
    while (*str && cnt < len) {
        unsigned char znak = *str;
        // Sprawd? czy UTF-8 i czy mamy miejsce w kolumnach
        if (znak & 0x80) {
            if (cnt >= len) break;  // ju? brak miejsca

            if (*(str + 1) == '\0') break; // niepe?ny UTF-8 na ko?cu

            uint16_t znak_utf = (znak << 8) | (uint8_t)*(str + 1);

            str += 2;

            switch (znak_utf) {
                case 0xC485: lcd_char('a'); break;
                case 0xC487: lcd_char('c'); break;
                case 0xC499: lcd_char('e'); break;
                case 0xC582: lcd_char('l'); break;
                case 0xC584: lcd_char('n'); break;
                case 0xC3B3: lcd_char('o'); break;
                case 0xC59B: lcd_char('s'); break;
                case 0xC5BA:
                case 0xC5BC: lcd_char('z'); break;
                case 0xC484: lcd_char('A'); break;
                case 0xC486: lcd_char('C'); break;
                case 0xC498: lcd_char('E'); break;
                case 0xC581: lcd_char('L'); break;
                case 0xC583: lcd_char('N'); break;
                case 0xC393: lcd_char('O'); break;
                case 0xC59A: lcd_char('S'); break;
                case 0xC5B9:
                case 0xC5BB: lcd_char('Z'); break;
...
void lcd_utf8str_padd_rest(const char* str, const uint16_t len, char padd) {
	uint8_t rest = lcd_utf8str_part(str, len);
	while (rest) {
		lcd_char(padd);
		rest--;
	}
